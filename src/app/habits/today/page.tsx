...
export default function Page() {
    const [habitsData, setHabitsData] = useState<{ [offset: number]: HabitCardProps[] }>({});
    ...
    useEffect(() => {
        if (Object.keys(habitsData).length > 0) { sectionsRef.current[todayIndex]?.scrollIntoView({ behavior: "smooth" }); }
    ...
    const handleScroll = useCallback(() => {
        ...
        sectionsRef.current.forEach((section, index) => {
            if (section) {
                const distance = Math.abs(section.getBoundingClientRect().top - containerRect.top);
            ...
            <div className="w-full flex absolute top-0 left-0 mt-20 h-[calc(100vh-5rem)] overflow-hidden">
                <aside className="bg-gray-50 p-4 flex flex-col justify-center">
                    <div className="space-y-4">
                        {relativeDays.map((_, index) => ( <div key={index} className="w-12 h-12 rounded-full bg-gray-300 animate-pulse" /> ))}
                    </div>
                </aside>

                <div className="flex-1 overflow-y-auto p-6 bg-gray-50 grid gap-6">
                    {relativeDays.map((_, index) => (
                        <div key={index} className="min-h-screen border-b border-gray-200 snap-start space-y-4">
                            <div className="w-1/2 h-8 bg-gray-300 animate-pulse" />
                            <div className="w-1/3 h-6 bg-gray-300 animate-pulse" />
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {Array(4)
                                    .fill(0)
                                    .map((_, i) => ( <div key={i} className="w-full h-32 bg-gray-300 animate-pulse" /> ))}
                            </div>
                        </div>
                    ))}
                </div>
    ...
    return (
        <div className="w-full flex absolute top-0 left-0 mt-20 h-[calc(100vh-5rem)] overflow-hidden">
            <aside className="bg-gray-50 p-4 flex flex-col justify-center">
                <div className="space-y-4">
                    {relativeDays.map((day, index) => (
                        <button key={day.offset} onClick={() => scrollToDay(index)}
            ...
            <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-6 bg-gray-50 snap-y snap-mandatory" >
                {relativeDays.map((day, index) => {
                    ...
                    return (
                        <motion.section
                            ...
                            className="min-h-screen pt-12 pb-20 border-b border-gray-200 snap-start"
                            initial="hidden"
                            animate="visible"
                            variants={dayVariants}
                        >
                            <h2 className="text-3xl font-semibold mb-4 text-orange-700 capitalize">{day.label}</h2>
                            <p className="text-lg mb-6 text-gray-500">{habits.length} szokás található</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
                                {habits.map((habit: HabitCardProps) => ( <HabitCard key={habit.habit_id} {...habit} /> ))}
                            </div>
                        </motion.section>
                    );
                })}
